...
<time>=), =headline= (which is also the default when no =comment=
property is present), which uses the headline as the comment, =path=,
which uses the headline together with the whole path, and any other
string, which is then used as a comment.
